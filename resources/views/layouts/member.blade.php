...
<body class="font-sans bg-stone text-ink antialiased min-h-screen flex flex-col">
    ...
    <header class="bg-cream border-b border-line">
        <div class="max-w-5xl mx-auto flex items-center justify-between h-16 px-5">
            <a href="{{ route('member.dashboard') }}" class="flex items-center gap-2.5">
                <img src="{{ asset('images/myf/logo-my-financials.png') }}" alt="MY Financials" class="h-10 w-auto">
                <span class="font-serif font-semibold text-lg text-rust leading-none">Area Member</span>
            </a>
        ...
        @if ($member)
            <nav class="max-w-5xl mx-auto px-5 flex gap-1 -mb-px overflow-x-auto">
                @php($tabs = [
                ...
                @foreach ($tabs as $t)
                    <a href="{{ route($t['route']) }}"
                        class="px-4 py-3 text-sm font-semibold border-b-2 transition {{ request()->routeIs($t['active']) ? 'border-rust text-rust' : 'border-transparent text-muted hover:text-rust' }}">
    ...
    <footer class="px-5 py-6 border-t border-line text-center text-xs text-muted">
        &copy; {{ date('Y') }} {{ config('app.name') }}
        <span class="mx-1">·</span>
        <a href="{{ route('home') }}" class="hover:text-rust font-semibold">Kembali ke situs</a>
    </footer>
</body>
